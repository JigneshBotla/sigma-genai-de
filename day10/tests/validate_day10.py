...
# ── Dynamic ChromaDB & RAG Mock Hook for Python 3.14+ ─────────────────────────
class DummyChromaConfig(BaseModel):
    provider: Literal['chromadb'] = 'chromadb'

mock_chromadb_config = MagicMock()
mock_chromadb_config.ChromaDBConfig = DummyChromaConfig
sys.modules['crewai.rag.chromadb.config'] = mock_chromadb_config
sys.modules['crewai.rag.chromadb.types'] = MagicMock()
sys.modules['crewai.rag.chromadb'] = MagicMock()

class DummyEmbeddingClass:
    pass
...
class MockEmbeddingModule(ModuleType):
    def __init__(self, name):
        super().__init__(name)
    def __getattr__(self, name):
        return DummyEmbeddingClass

class MockApiTypesModule:
    def __getattr__(self, name):
        if name == 'EmbeddingFunction':
            return DummyEmbeddingFunction
        return DummyEmbeddingClass

class ChromadbEmbeddingFunctionsFinder:
    def find_spec(self, fullname, path, target=None):
        if fullname.startswith('chromadb.utils.embedding_functions.'):
            return ModuleSpec(fullname, self)
        return None

    def create_module(self, spec):
        return MockEmbeddingModule(spec.name)

    def exec_module(self, module):
        pass

sys.meta_path.insert(0, ChromadbEmbeddingFunctionsFinder())

sys.modules['chromadb'] = MagicMock()
sys.modules['chromadb.config'] = MagicMock()
sys.modules['chromadb.utils'] = MagicMock()
sys.modules['chromadb.utils.embedding_functions'] = MockEmbeddingModule('chromadb.utils.embedding_functions')
sys.modules['chromadb.api'] = MagicMock()
sys.modules['chromadb.api.client'] = MagicMock()
sys.modules['chromadb.api.configuration'] = MagicMock()
sys.modules['chromadb.api.types'] = MockApiTypesModule()

# ── AST compatibility patches for Python 3.14+ ────────────────────────────────
import ast
